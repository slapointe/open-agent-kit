...
Tasks define what to do (default_task, maintained_files, oak_queries).
...
# Path to the definitions directory within agent_runtime
# Path: agent_runtime/registry.py -> agent_runtime/
_FEATURE_ROOT = Path(__file__).parent
_AGENTS_DIR = _FEATURE_ROOT / AGENTS_DEFINITIONS_DIR
...
class AgentRegistry:
    ...
    def _load_task(self, yaml_file: Path, is_builtin: bool = False) -> AgentTask | None:
        ...
        # Parse oak_queries (with backward compat for "ci_queries" key)
        oak_queries: dict[str, list[OakQueryTemplate]] = {}
        for phase, queries in data.get("oak_queries", data.get("ci_queries", {})).items():
            oak_queries[phase] = []
            for q_data in queries:
                if isinstance(q_data, dict):
                    oak_queries[phase].append(OakQueryTemplate(**q_data))
    ...
    def _load_agent(self, definition_file: Path) -> AgentDefinition | None:
        ...
        tool_access_data = data.get("tool_access", data.get("ci_access", {}))
        tool_access = AgentToolAccess(
            code_search=tool_access_data.get("code_search", True),
            memory_search=tool_access_data.get("memory_search", True),
            session_history=tool_access_data.get("session_history", True),
            project_stats=tool_access_data.get("project_stats", True),
            sql_query=tool_access_data.get("sql_query", False),
            memory_write=tool_access_data.get("memory_write", False),
        )
