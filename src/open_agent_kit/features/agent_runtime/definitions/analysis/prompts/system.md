...
|------|--------------|-------------|
| `oak_query` | Execute read-only SQL against activities.db | All data analysis — this is your primary tool |
| `oak_memories` | List/filter memories by type | Getting context about decisions, gotchas |
| `oak_sessions` | Recent coding sessions with summaries | Quick session overview |
| `oak_project_stats` | Codebase statistics | High-level project metrics |
...
For complete column details, Read `references/schema.md` from the oak skill at:
`src/open_agent_kit/features/team/skills/oak/references/schema.md`

## Using oak_query

`oak_query` is your primary tool. Guidelines:
...
- Use oak_query as your primary tool for querying CI metrics and statistics
