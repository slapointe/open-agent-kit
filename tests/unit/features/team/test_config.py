...
def mock_machine_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "open_agent_kit.features.team.activity.store.backup.get_machine_identifier",
...
class TestGetConfigOrigins:
    ...
    def test_user_origin_for_overlay_keys(self, tmp_path, mock_machine_id):
        ...
        oak_dir = tmp_path / ".oak"
        oak_dir.mkdir()
        (oak_dir / "config.yaml").write_text(yaml.dump({"team": {}}))
